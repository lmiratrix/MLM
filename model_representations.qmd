...
For a running example, say we have a collection of schools that we have randomized into treatment and control conditions.
 ...
 * We subscript school-level covariates with only a $j$ vs. individual-level covariates get an $ij$. If you want, you can index everything by $ij$; the fact that $S_{ij}$ will then be the same for all students $i$ in school $j$ is hidden in the data.
...
$$
 * You can call all the different pieces by different letters to indicate whether you care about them or not.
...
### The bracket-subscript notation from Gelman and Hill
