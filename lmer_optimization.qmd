---
title: "Error Messages and Optimization Algorithms for MLMs"
author: "Luke Miratrix and prior TFs (Marcus Waldman)"
editor: 
  markdown: 
    wrap: sentence
---
...
Many of these relate to "failure to converge." Others carry on about "Singular fits." In this chapter, we'll discuss what these things mean and how to address it.


## What is a "singular fit"?

Sometimes when fitting a model, especially with random slopes or multiple random effects, the lmer output shows "boundary (singular) fit: see ?isSingular" in red color.
What does _that_ mean?

If you check out R's help page, it says "While singular models are statistically well defined (it is theoretically sensible for the true maximum likelihood estimate to correspond to a singular fit), there are real concerns that (1) singular fits correspond to overfitted
...
The short version is, if you think your model is correctly specified, singular fits most often occur when R is trying to estimate something beyond the plausible value, for example a negative variance or a correlation outside of the -1 to 1 range.
This can happen if, say, there is little variation in your cluster means, and you just happen to get observed cluster means that are more close together than you would expect by random chance--in this case, the model's best fit is that the clusters are kind of pulling together, i.e., the variance is _negative_ (which is impossible).

So what do you do? In principle, you can drop a random effect that seems less useful, and it might go away.
Or you can just live with the warning---the fixed effect standard errors will probably still be about right.
...
For #1, calculate the standard deviation of all your variables and compare them to one another.
If you have one variable with a very large sd relative to the others, try rescaling it.
