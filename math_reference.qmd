...
But really you should open the .Rmd file and cut and paste the relevant equations into your own work, and then modify as necessary.
...
To write an equation on a line by itself, surround the math stuff with the double "\$\$" and "\$\$".
E.g., if we write:
...
If we want multiple lines, we use a double backslash ("\\\\") to denote a line break (even if we have a line break we have to do this---we have to explicitly tell the program converting our raw text to nice formatted text where the line breaks are).
E.g., if we write:
...
Notice how these equations are not quite lined up nicely.
It automatically centers each line.
We can make it nicer using the `\begin{aligned}` and `\end{aligned}` commands to get our equations to line up.
We then, inside the begin-end block of math, line things up by the `&` symbols on each row of our equation:
